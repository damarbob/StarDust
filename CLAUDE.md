...
- **[src/StarDust.php](src/StarDust.php):** `StarDust\StarDust` holds the injected `Config` and exposes typed accessors (`pdo()`, `logger()`, `config()`) plus:
  ...
  - The model rename entry point — `renameModel(int $tenantId, int $modelId, string $newName): void`. Synchronous and complete on return, unlike `renameField()`: a model name is a label rather than an identity (entries, slots, filters and exports all key on `model_id`, and no snapshot caches the name), so it is one UPDATE with no backfill and no window. Deliberately does **not** bump `stardust_schema_version` — nothing cached changes and the version row is a singleton — which is pinned by an explicit negative assertion so it cannot be quietly reversed. Backed by `Rename\ModelRenamer`, the one registry collaborator that takes no clock. Details, including why `schemaBuilder()` still finds models by name and therefore creates a duplicate after a rename: `src/Rename/CLAUDE.md`.
  - The ADR 0036 rename entry point — `renameField(int $tenantId, int $fieldId, string $newName): void`. Returns as soon as the registry commits; the payload rewrite is asynchronous and **needs a running Reconciler**. Because `entry_data.fields` is keyed by field name, a rename is a rewrite of every entry in the model, not a registry write. Details, and the list of which surfaces bridge the mid-rename window (reads, point reads, writes and CSV export do; filters and the JSON export artifact deliberately do not): `src/Rename/CLAUDE.md`.
...
| [src/Rename/](src/Rename/) | ADR 0036 field rename: registry flip + chunked `entry_data` payload rewrite. Touches no slot. | — |
...
- ADR 0036 rename tests live under [tests/Smoke/Rename/](tests/Smoke/Rename/) and extend `Phase6bTestCase`, which gained `makeRenameInitiator()`, `makeRenameBackfillWorkSource()`, `runRenameTick()` (one tick, so a test can stop the drain half-migrated), `fetchRenameCheckpointForField()`, and `rawPayload()` (the stored payload with no assembler and no fallback — the only way to see which key a row is actually on). **There is no `Phase6cTestCase`; a rename is not a phase.** Two fixture traps: `MySQL normalises JSON object key order on storage`, so payload assertions must `ksort()` both sides rather than assert on order; and `compactModel()` returns an empty plan for an unfragmented model, so a compaction-guard test must strand a slot on a second page by direct registry UPDATE or it passes vacuously (same documented bypass as `SlotAffinityTest`).
