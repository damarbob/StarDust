...
| 5 | Resilience daemons — singleton Watcher with capacity- and demand-driven provisioning under `GET_LOCK` (ADR 0035) plus the ADR 0019 cardinality advisory; multi-worker Reconciler draining `stardust_sync_queue` and `stardust_import_jobs` via SKIP LOCKED, with ADR 0018 DLQ + operator replay. | `src/Daemon/`, `src/Watcher/`, `src/Reconciler/` |
...
| [src/Slot/](src/Slot/) | `SlotReserver` (the ADR 0034 filterability chokepoint) and `IndexedSlotPredicate`. | 2 |
...
| [src/Watcher/](src/Watcher/) | Singleton page provisioner + `CardinalitySampler`. Demand-driven since ADR 0035. | 5 / 6b |
| [src/Reconciler/](src/Reconciler/) | Multi-worker drain over three work sources, plus the ADR 0018 DLQ and replayer. | 5 |
...
- **When a phase or stage ships, update the docs in the same change:** this file, the affected `src/*/CLAUDE.md`, [README.md](README.md) (no ADR citations), [CHANGELOG.md](CHANGELOG.md), and [TESTING.md](TESTING.md) if coverage changed. New event names additionally require an ADR 0020 update or `EventVocabularyTest` fails.
