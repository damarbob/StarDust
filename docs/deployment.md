...
A large export therefore completes over several cron firings rather than one — each firing's `tick` picks up wherever the last one's budget ran out, the same way it already resumes any other in-progress work. **Read the disk-pressure caveat below before turning this on**: the pre-claim gate that is supposed to stop a nearly-full disk from accepting new export work checks partition-level free space, and shared hosting commonly enforces a per-account quota above the filesystem layer that the gate cannot see — a quota'd account can read as "plenty of space" right up until every write fails. This is a documented gap, not a resolved one; verify your host's behaviour (or watch for `failed:disk_full` jobs) before relying on the gate to protect you.
