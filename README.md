...
- **No delete for models or fields.** The rest of the definition layer is covered — `schemaBuilder()` registers, `listModels()` / `describeModel()` introspect, `renameField()` and `renameModel()` rename, `retypeField()` changes a field's type online, and `promoteFieldToFilterable()` / `demoteFieldFromFilterable()` turn indexing on and off — but there is no entry point for removing a model or field. The practical workaround for an unwanted field is to demote it (which releases its slot); a JSON-only field occupies no slot, so leaving it behind costs you nothing but a key in the payload. Editing `stardust_models` / `stardust_fields` by hand is not supported, though foreign keys will at least refuse to drop a field that still holds a live slot, so a mistake there fails loudly rather than corrupting your slot inventory. A first-class definition API covering the full lifecycle is on the roadmap.
