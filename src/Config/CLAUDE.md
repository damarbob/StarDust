...
- `$cardinalityJitterSeconds` (defaults to 10% of `$cardinalityIntervalSeconds` = 8_640) — **2026-06-20 Gap 7 resolution.** The randomized ± window the Watcher draws a fresh offset from on each periodic cardinality sample, and the full-interval span it phase-randomizes the FIRST sample across, so a fleet of singleton Watchers started in lockstep does not stampede on the same daily schedule. The RNG itself is an injectable `Watcher` constructor seam — `random_int` in production.
