...
- `$pageIndexHeadroom` (**4**) — **ADR 0042.** Columns of every slot family a newly provisioned page indexes, over and above current demand, so a new page carries sixteen indexed columns instead of the one to three that current demand justifies. **Since ADR 0043 that is also the page's whole column count** — a page is created with exactly what it indexes, so this knob sets capacity, not just how much of it is usable. It reaches `ProvisioningPlanner` as a `Watcher\IndexHeadroomPolicy` rather than an `int`, because ADR 0042 leaves the *rule* open between a flat `k`, a per-family table and a registry-derived count — `FlatIndexHeadroom` is the shipped implementation and the others are new classes, not new fields. **Fixed per page at creation** (ADR 0012), so changing it never widens a page that already exists. Not range-validated, and it does not need to be: the planner clamps to `[0, family capacity]` and applies its own floor of one column per demanded family, so no value can starve a waiter or name a column that does not exist. `0` degrades to demand-sizing rather than to starvation, but it is **not** an exact return to the pre-0042 policy: for a family nobody is waiting on, that policy provisioned a page with no indexes, which under ADR 0043 is a page with no columns — the planner declines to provision one at all.

**Four fields between `$spreadExcessPageThreshold` and `$pageIndexHeadroom` are missing from this list** — `$modelPurgeChunkSize`, `$modelPurgeLockRetryBudget`, `$reconcilerLockRetryBudget` and `$reconcilerLockRetryDelayMicros`, in constructor order. They are documented in the root `CLAUDE.md` and in `src/Reconciler/CLAUDE.md`; the gap is noted here so the "in the order shown" claim above is not read as a complete inventory.
