...
**The manifest is hoisted into typed `?int $chunks` / `?int $entriesWritten`; the raw array is not exposed.** `ExportJob`'s rule is *engine-generated envelopes get typed fields, consumer-supplied payloads pass through verbatim* — that is why `model_id` is hoisted but `.filter` is not. The import manifest is written only by `ImportJobWorkSource`, in a closed `{chunks, entries_written}` shape a consumer never supplies, so it falls on the typed side.
...
**On a `failed` job, `entriesWritten` is the replay boundary.** The manifest is checkpointed inside every chunk transaction (`ImportJobWorkSource`) and `failJob()` never overwrites it, so the last committed count survives the failure. That is the single most useful thing this DTO reports and what `GetImportJobTest` pins hardest.
...
**Known divergence from ADR 0011 §26**, tracked as ROADMAP item 13: the ADR specifies a manifest enumerating per-chunk outcomes (`committed | rolled_back | failed`), entity ID ranges, and failure reasons. What ships is a resume checkpoint. Widening it is not a DTO change — `entries_written` is load-bearing for the Reconciler's abandoned-claim resume.
