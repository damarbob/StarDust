...
- `BackfillExecutor::backfill(int $entryId): BackfillResult` — the Phase 5 sync-queue drain helper. Loads `entry_data`, runs `LiveSlotMap` + `PayloadSplitter`, calls `SlotRowUpserter` per page. It does NOT touch `entry_data` or `stardust_sync_queue` (the Reconciler owns the queue). Throws `EntryDataMissingException` when the queued `entry_id` no longer has a backing row — the Reconciler routes that to DLQ.
